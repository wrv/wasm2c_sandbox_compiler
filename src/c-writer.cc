...
void CWriter::Write(const Const& const_) {
  switch (const_.type()) {
    ...
    case Type::V128: {
      // NOTE: lane_type() doesn't seem to get set when reading from a binary WASM
      //       file, so we'll just default to i32x4 constant type for now
...
void CWriter::Write(const ConvertExpr& expr) {
  switch (expr.opcode) {
    ...
    case Opcode::I32X4TruncSatF64X2UZero:
      WriteSimpleUnaryExpr(expr.opcode, "simde_wasm_u32x4_trunc_sat_f64x2_zero");
      break;
...
void CWriter::Write(const UnaryExpr& expr) {
  switch (expr.opcode) {
    ...
    case Opcode::I16X8ExtaddPairwiseI8X16S:
      WriteSimpleUnaryExpr(expr.opcode, "simde_wasm_i16x8_extadd_pairwise_i8x16");
      break;

    case Opcode::I16X8ExtaddPairwiseI8X16U:
      WriteSimpleUnaryExpr(expr.opcode, "simde_wasm_u16x8_extadd_pairwise_u8x16");
      break;
    ...
    case Opcode::I32X4ExtaddPairwiseI16X8U:
      WriteSimpleUnaryExpr(expr.opcode, "simde_wasm_u32x4_extadd_pairwise_u16x8");
      break;
  ...
  switch (expr.opcode) {
    ...
    case Opcode::I8X16ReplaceLane:{
      Write(StackVar(1, result_type), " = simde_wasm_i8x16_replace_lane(",
            StackVar(1), ", ", expr.val, ", ", StackVar(0), ");",
            Newline());
      DropTypes(2);
      break;
    }
